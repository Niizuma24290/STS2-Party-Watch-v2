...
| Phase 9A | 代码完成，待 Steam 验证 | 前端 UI、设置与显示生命周期 | HUD 可见性、冻结快照、设置面板、位置与颜色项已构建/发布/安装通过 | 视觉微调 |
...
追加修补：`TungstenRod` / `BeatingRemnant` 共存时，Party Watch 预测固定先应用 `TungstenRod`，再应用 `BeatingRemnant`；本轮等待用户 Steam 运行时验证。
