...
| Phase 9B | 代码已接入，待专项运行时验证 | Poison 行动前存活预览 | 普通 Poison 确定击杀的 enemy instance 不再贡献当前 Attack Intent；build / publish 通过 | 专项 Steam 验证 |
...
多人本机 HUD 收口：Workshop 订阅版已确认本机 `LocalSinglePlayerHud` 可在多人战斗中显示于本机角色血条下方；左上角队友蓝盾属于原生队友状态 / Block 显示，不作为 Party Watch 队友预测；`FormalMultiplayerHud` 继续冻结。
