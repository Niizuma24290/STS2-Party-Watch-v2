...
| Burn-style hand turn-end `DamageVar` | Implemented, current runtime defect reported | `🛡`, total `-N` | `HasTurnEndInHandEffect`, `CreatureCmd.Damage`, `DamageVar`, `Hook.ModifyDamage(...)` | `CardTurnEndDamageInspector`, `TryReadHandTurnEndDamage`, `TryReadOrderedHandTurnEndEvents` | Historical Burn runtime evidence; current v0.3.0 user report says all damage-dealing Status/Curse cards hide the complete HUD | `HpLossVar` and unblockable props do not enter `🛡`; exact current card matrix and root cause are pending. |
...
| Beckon | Implemented exact reader, included in broader current defect | `♥`, total `-N` | `HpLossVar(6)`, unblockable turn-end hand effect | `VerifiedFixedTurnEndHpLossReader` | Current v0.3.0 family-level user report covers damage-dealing Status/Curse cards; no separate refreshed Beckon matrix captured | The exact-reader classification path remains a diagnosis candidate; do not claim current full-path RuntimeVerified. |
| Bad Luck | Implemented exact reader, included in broader current defect | `♥`, total `-N` | `HpLossVar(13)`, unblockable turn-end hand effect | `VerifiedFixedTurnEndHpLossReader` | Earlier individual reproduction plus current v0.3.0 family-level user report | The current scope is not limited to Bad Luck; complete root cause remains pending. |
| Regret | Implemented exact reader, included in broader current defect | `♥`, total `-N` | hand count captured before turn-end effect | `VerifiedFixedTurnEndHpLossReader` | Earlier individual reproduction plus current v0.3.0 family-level user report | The current scope is not limited to Regret; complete root cause remains pending. |
