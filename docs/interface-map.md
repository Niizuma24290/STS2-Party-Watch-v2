...
| 基础机制已运行时验证；惊涛组合待验证 | 读取波纹水盆预期 Block | `player.Relics.OfType<RippleBasin>()` + `CombatManager.Instance.History.CardPlaysFinished` + `localCreature.GetPower<StampedePower>()` + `CardPile.Get(PileType.Hand, player)` | 公开只读 relic 集合 / combat history / power / hand pile 读取 | 基础水盆是；水盆 + 惊涛待验证 | 本回合已打出本机 Attack 或惊涛即将自动打出非 `Unplayable` Attack 时，不计入水盆 Block；不模拟无合法目标等自动打牌边界。 |
